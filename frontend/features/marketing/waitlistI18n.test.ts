...
describe("waitlist i18n coverage", () => {
  ...
  it("defines a waitlist namespace in every locale", () => {
    for (const [name, locale] of Object.entries(locales)) {
      expect(locale.waitlist, `locale ${name} missing waitlist`).toBeDefined();
    }
  });

  it("uses identical keys across locales", () => {
    const reference = flattenKeys(en.waitlist as Json).sort();
    expect(reference.length).toBeGreaterThan(0);
    for (const [name, locale] of Object.entries(locales)) {
      const keys = flattenKeys(locale.waitlist as Json).sort();
  ...
  it("has no empty strings", () => {
    for (const [name, locale] of Object.entries(locales)) {
      for (const value of leafValues(locale.waitlist as Json)) {
