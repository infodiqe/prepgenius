# claude_rules.md — PrepGenius AI Development Rules (Claude Code)

> **Scope:** This file governs how Claude Code works in the PrepGenius AI repository. Sections 1–11 (the engineering rules) are the **single source of truth** and are byte-for-byte identical to `codex_rules.md`; only the *Operating Model* below is tailored to Claude Code. If you change an engineering rule, change it in both files.

---

## Operating Model (Claude Code)

1. **Read context before writing code.** Before any task, read: `PRD v4`, the `System Architecture Document`, and the `Database Design`. Code that doesn't trace to these is out of scope.
2. **Plan, then act in small diffs.** State a short plan, implement the smallest correct change, prefer editing existing files over creating new ones, and keep diffs reviewable.
3. **Migrations and tests are part of "writing code," not afterthoughts.** Any schema touch ⇒ a Django migration in the same change. Run the test suite and migrations before declaring done.
4. **Use Django Admin and management commands** for internal/ops tooling instead of bespoke UIs.
5. **Verify before destructive actions.** Never drop tables, reset migrations, delete data, rewrite history, or remove files without explicit confirmation.
6. **Ask when uncertain — never guess past a golden rule.** If a request conflicts with the rules below or the PRD, stop and surface it rather than silently diverging.
7. **Stay in scope.** No drive-by refactors, dependency bumps, or "while I'm here" changes unless the task requires them.

---

## 0. Golden Rules (non-negotiable)

These override convenience, cleverness, and any request to "just this once" skip them. If asked to break one, refuse and escalate.
...
Stop and ask a human when a task would:
